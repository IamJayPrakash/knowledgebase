...
// Strategy: DP: rob = max(prev1, prev2 + num) in O(1) space

function solution_198(inputData) {
    ...
    for (let i = 0; i < inputData.length; i++) {
        ...
        // Step 3: Validate optimal criteria based on DP: rob = max(prev1, prev2 + num) in O(1) space
...
def solution_198(input_data):
    # Step 1: Initialize required data structure or pointers for DP: rob = max(prev1, prev2 + num) in O(1) space
    state = {}

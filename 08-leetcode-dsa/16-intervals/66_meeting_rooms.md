...
// Strategy: Sort by start time, check if intervals[i][0] < intervals[i-1][1]

function solution_252(inputData) {
    ...
    for (let i = 0; i < inputData.length; i++) {
        ...
        // Step 3: Validate optimal criteria based on Sort by start time, check if intervals[i][0] < intervals[i-1][1]
...
def solution_252(input_data):
    # Step 1: Initialize required data structure or pointers for Sort by start time, check if intervals[i][0] < intervals[i-1][1]
    state = {}
